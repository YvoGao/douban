...
def load_dataset():
    ...
    movies = []
    # 获得训练值
    csv_file = open(r'../prelabel.csv', 'r')
...
if __name__ == '__main__':
    ...
    for i in range(len(p)):
        # print(t[i][0][0])
        loss = abs(float(p[i]) - float(t[i][0][0]))
        losses.append(loss)
    Loss = (
        Line(init_opts=opts.InitOpts(bg_color='rgb(255,255,255)', theme=ThemeType.INFOGRAPHIC))
            .add_xaxis(movies)
            .add_yaxis("loss值", losses)
            .set_global_opts(title_opts=opts.TitleOpts(title="预测集结果", subtitle=""),
            ...
            .set_series_opts(label_opts=opts.LabelOpts(is_show=False),
                             markline_opts=opts.MarkLineOpts(
                                    data=[
                                        opts.MarkLineItem(type_="average", name="平均值")          #插入y轴=0的标记线
                                    ]
                            )
            )
    )
    Loss.render(r'../logs/losses.html')
    Line = (
        Line(init_opts=opts.InitOpts(bg_color='rgb(255,255,255)', theme=ThemeType.INFOGRAPHIC))
            .add_xaxis(movies)
            .add_yaxis("真实评分", t)
    ...
    Line.render(r'../logs/prenewresult.html')
